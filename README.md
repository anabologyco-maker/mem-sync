...
`mem-sync` gives Codex, Claude Code, and OpenCode one project-scoped durable
memory without making any harness authoritative. It synchronizes the context
that can safely be made portable: project instructions, Claude Code auto-memory,
and Codex's generated local-memory output. OpenCode consumes the shared
`AGENTS.md` directly. It does **not** copy credentials, hidden product system
prompts, permission policy, or arbitrary transcripts into model context.
...
4. Adds a versioned shared-memory protocol that tells all three harnesses how
   to remember, forget, and clean durable facts.
5. Makes `AGENTS.md` and `CLAUDE.md` hard links to
   `.mem-sync/memory/MEMORY.md` when the filesystem permits it.
6. At a Git root, links Claude Code's native project auto-memory directory to
   the same canonical directory.
7. Registers the exact project root for background reconciliation and Codex
   native-memory harvesting.

Projects enabled by v0.1 receive the protocol once during their first v0.2 sync.
The upgrade is lossless and versioned in `.mem-sync/state.json`; it is not
continually reinserted after an agent deliberately rewrites the file.
...
- A normal edit from any agent is a replacement of the canonical state.
...
Codex next launch / OpenCode next attempt: [loads the cleaned version]
```
...
`.claude/CLAUDE.md`. v0.2 preserves but does not rewrite those files. Consolidate
...
OpenCode already provides a native `/move` session command and API. mem-sync
leaves OpenCode session movement to that supported interface instead of
rewriting its live SQLite database.
...
- Built-in harness system prompts are product-owned runtime
...
- v0.2 synchronizes the root instruction pair and supported native project
  memories. It inventories nested rules, skills, hooks, settings, subagents, and
  prompt customizations but does not translate them; executable/configuration
  state requires an explicit trust-aware adapter rather than blind file copying.
